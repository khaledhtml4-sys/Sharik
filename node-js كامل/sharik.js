...
// ─── مراقبة الأخطاء (اختيارية): تُفعَّل عند وجود SENTRY_DSN وتثبيت @sentry/node ───
try {
  ...
  console.warn("⚠️ SENTRY_DSN set but @sentry/node not installed — run: npm i @sentry/node");
}
...
// Native Gzip Compression Middleware (Zero-dependency) — يغطي الـAPI والملفات الثابتة عبر اعتراض write/end
const zlib = require("zlib");
const COMPRESSIBLE_RE = /text\/|application\/json|application\/javascript|application\/manifest\+json|image\/svg\+xml/i;
app.use((req, res, next) => {
  // socket.io يدير ترميزاته بنفسه، وRange طلبات (فيديو) لا تُضغط
  const ae = req.headers["accept-encoding"] || "";
  if (!ae.includes("gzip") || req.headers.range || req.url.startsWith("/socket.io/")) return next();

  const origWrite = res.write.bind(res);
  const origEnd = res.end.bind(res);
  let gzip = null;

  const chunkLen = (chunk) =>
    chunk === undefined || chunk === null ? 0
      : Buffer.isBuffer(chunk) ? chunk.length
        : (typeof chunk === "string" ? Buffer.byteLength(chunk) : 0);

  const tryStartGzip = (len) => {
    if (gzip) return true;
    if (res.headersSent || res.getHeader("Content-Encoding")) return false;
    if (!len || len < 1024) return false; // الاستجابات الصغيرة لا تستحق الضغط
    const ct = String(res.getHeader("Content-Type") || "");
    if (!COMPRESSIBLE_RE.test(ct)) return false;
    res.removeHeader("Content-Length"); // الطول يتغير بعد الضغط
    res.setHeader("Content-Encoding", "gzip");
    res.setHeader("Vary", "Accept-Encoding");
    gzip = zlib.createGzip();
    gzip.on("data", (c) => origWrite(c));
    gzip.on("end", () => origEnd());
    return true;
  };

  const normArgs = (a, b, c) => {
    if (typeof b === "function") return [a, undefined, b];
    return [a, b, c];
  };

  res.write = function (chunk, enc, cb) {
    [chunk, enc, cb] = normArgs(chunk, enc, cb);
    if (tryStartGzip(chunkLen(chunk)) || gzip) {
      const r = gzip.write(chunk, enc, cb);
      if (r === false) {
        // إعادة بث drain على res حتى يستأنف pipe المصدر (منع التعليق)
        gzip.once("drain", () => res.emit("drain"));
      }
      return r;
    }
  ...
  res.end = function (chunk, enc, cb) {
    [chunk, enc, cb] = normArgs(chunk, enc, cb);
    if (tryStartGzip(chunkLen(chunk))) {
      if (chunk) gzip.write(chunk);
      gzip.end();
      return;
    }
    if (gzip) {
      if (chunk) gzip.write(chunk, enc);
      gzip.end();
      return;
    }
...
const { getSkillsList, isTestableSkill } = require("./modules/skillQuestionBank");
...
app.use(express.static(path.join(__dirname, "../public"), {
  maxAge: "1d", // كاش يوم للـjs/css/الصور — يمنع إعادة التحميل في كل زيارة
  setHeaders: (res, filePath) => {
    // HTML وملفات الإعداد والسيرفس وركر تبقى بلا كاش (إعادة تحقق رخيصة 304 فقط)
    if (/.html$|sw.js$|config.js$|manifest.webmanifest$/.test(filePath)) {
...
// تنقية موحّدة لقوائم المهارات في كل المسارات
function normalizeSkillList(list) {
  if (!Array.isArray(list)) return null;
  return [...new Set(list.map((s) => String(s).trim().slice(0, 60)).filter(Boolean))].slice(0, 20);
}
...
function parseChatMembers(chatId) {
  ...
  // chatId = "<email1>_<email2>" — each email holds exactly one "@", but may
  // itself contain "_", so split on "_" then rejoin around the @ boundary.
  const raw = chatId.split("_").map((item) => item.trim()).filter(Boolean);
  if (raw.length < 2) return [];
  const atIdx = raw.findIndex((f) => f.includes("@"));
  if (atIdx === -1 || atIdx === raw.length - 1) return [];
  const email1 = raw.slice(0, atIdx + 1).join("_");
  const email2 = raw.slice(atIdx + 1).join("_");
  // كل بريد يجب أن يحتوي "@" واحدة بالضبط — وإلا فالمعرّف غير صالح
  const atCount = (s) => (s.match(/@/g) || []).length;
  if (atCount(email1) !== 1 || atCount(email2) !== 1) return [];
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (String(username1).trim().length > 50 || String(username2).trim().length > 50) {
...
app.post("/api/auth/send-otp", authMiddleware, async (req, res) => {
  try {
    ...
    if (user.isEmailVerified) {
      return res.json({ success: true, message: "البريد الإلكتروني مفعّل بالفعل" });
    }

    // توليد كود عشوائي مكون من 6 أرقام
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    user.emailOtp = crypto.createHash("sha256").update(otp).digest("hex");
    user.emailOtpExpires = new Date(Date.now() + 15 * 60 * 1000); // 15 دقيقة
    await user.save();

    if (isEmailConfigured()) {
      await sendMail({
        to: user.email,
        subject: "رمز تفعيل البريد الإلكتروني — شارك",
        text: `رمز التفعيل الخاص بك هو: ${otp}\nالرمز صالح لمدة 15 دقيقة.`,
        html: `<h3>مرحباً ${user.username1}!</h3><p>رمز تفعيل بريدك في منصة شارك هو: <b style="font-size:24px;color:#3b82f6">${otp}</b></p><p>الرمز صالح لمدة 15 دقيقة.</p>`,
      });
    } else {
      console.log(`[Dev OTP] Code for ${user.email}: ${otp}`);
    }

    res.json({ success: true, message: "تم إرسال رمز التفعيل إلى بريدك الإلكتروني" });
  } catch (err) {
    console.error("send-otp error:", err);
    res.status(500).json({ error: "تعذر إرسال رمز التفعيل" });
  }
});
...
app.post("/api/auth/verify-otp", authMiddleware, async (req, res) => {
  try {
    ...
    if (!code || String(code).trim().length !== 6) {
      return res.status(400).json({ error: "يرجى إدخال رمز مكون من 6 أرقام" });
    }
    ...
    if (user.isEmailVerified) {
      return res.json({ success: true, message: "البريد مفعّل بالفعل" });
    }

    const hash = crypto.createHash("sha256").update(String(code).trim()).digest("hex");
    if (user.emailOtp !== hash || !user.emailOtpExpires || user.emailOtpExpires.getTime() < Date.now()) {
      return res.status(400).json({ error: "رمز التفعيل غير صحيح أو منتهي الصلاحية" });
    }
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    // 📍 تسجيل بيانات الجهاز والـ IP
    const rawIp = req.headers["x-forwarded-for"]?.split(",")[0]?.trim() || req.socket?.remoteAddress || req.ip || "";
    const ua = req.headers["user-agent"] || "";
    const isMobile = /mobile|android/i.test(ua) && !/ipad|tablet/i.test(ua);
    const isTablet = /ipad|tablet/i.test(ua) || (!/mobile/i.test(ua) && /android/i.test(ua));
    ...
    const loginEntry = { ip: rawIp, userAgent: ua.slice(0, 300), device: deviceType, browser, os, at: new Date() };
    user.loginHistory = [...(user.loginHistory || []).slice(-9), loginEntry];
...
// Update full profile (bio, avatar, country, names, experience, links, certificates, skills, onboarding)
app.put("/api/me/profile", authMiddleware, async (req, res) => {
  try {
    ...
    const updates = {};
    if (typeof username1 === "string" && username1.trim()) updates.username1 = username1.trim().slice(0, 50);
    if (typeof username2 === "string" && username2.trim()) updates.username2 = username2.trim().slice(0, 50);
    ...
    if (typeof country === "string") updates.country = country.slice(0, 100);
    if (typeof experience === "string") updates.experience = experience.slice(0, 2000);
    if (typeof onboarded === "boolean") updates.onboarded = onboarded;

    // نفس تنقية /api/skills — لا يمرر أي مسار قيمًا غير سليمة للمهارات
    const cleanTeach = normalizeSkillList(teachSkills);
    const cleanLearn = normalizeSkillList(learnSkills);
    if (cleanTeach) updates.teachSkills = cleanTeach;
    if (cleanLearn) updates.learnSkills = cleanLearn;

    if (Array.isArray(availability)) {
      updates.availability = availability
        .map((a) => ({
          day: String(a?.day || "").slice(0, 20),
          slots: Array.isArray(a?.slots) ? a.slots.map((s) => String(s).slice(0, 10)).slice(0, 12) : [],
        }))
        .filter((a) => a.day)
        .slice(0, 7);
    }
    ...
    if (Array.isArray(languages)) {
      updates.languages = languages.map((l) => String(l).trim()).filter(Boolean).slice(0, 10);
    }
...
// Real Analytics & Activity Heatmap for User Dashboard
app.get("/api/analytics/me", authMiddleware, async (req, res) => {
  try {
    ...
    const now = new Date();
    const startDate = new Date(now.getTime() - 140 * 24 * 60 * 60 * 1000); // last 140 days
    ...
    const heatmap = {};
    sessions.forEach((s) => {
      const dStr = new Date(s.createdAt || s.startAt).toISOString().slice(0, 10);
      heatmap[dStr] = (heatmap[dStr] || 0) + 1;
    });

    (user.activityTimeline || []).forEach((act) => {
      if (act.date) {
        const dStr = new Date(act.date).toISOString().slice(0, 10);
        heatmap[dStr] = (heatmap[dStr] || 0) + 1;
      }
    });
    ...
    const totalHours = completed.reduce((acc, s) => {
      if (s.startAt && s.endAt) {
        const dur = (new Date(s.endAt) - new Date(s.startAt)) / 3600000;
        return acc + (dur > 0 ? dur : 1);
      }
      return acc + 1;
    }, 0);
    ...
    res.json({
      ...
      stats: {
        ...
        completedSessions: completed.length || user.completedSessions || 0,
        totalHours: Math.round(totalHours * 10) / 10 || user.learningHours || 0,
        streak: user.dailyStreak || 0,
        verifiedSkillsCount: (user.verifiedSkills || []).length,
      },
      skillBreakdown: {
        teach: user.teachSkills || [],
        learn: user.learnSkills || [],
        verified: user.verifiedSkills || [],
      },
    });
  } catch (err) {
    console.error("Analytics error:", err);
    res.status(500).json({ error: "تعذر جلب التحليلات" });
  }
});

// GET /api/me/security — جلب سجل الدخول والأجهزة للمستخدم الحالي
app.get("/api/me/security", authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.userId).select("loginHistory lastLoginAt email").lean();
    if (!user) return res.status(404).json({ error: "مستخدم غير موجود" });
    res.json({
      loginHistory: (user.loginHistory || []).slice(-10).reverse(), // آخر 10 دخولات
      lastLoginAt: user.lastLoginAt,
      email: user.email
    });
  } catch (err) {
    res.status(500).json({ error: "خطأ في جلب بيانات الأمان" });
  }
});

// ─── GDPR: تصدير بيانات المستخدم (قابلية نقل البيانات) ──────────────
app.get("/api/me/export", authMiddleware, async (req, res) => {
  try {
    ...
    const [messages, matches, reports] = await Promise.all([
      Message.find({ $or: [{ sender: email }, { receiver: email }] }).sort({ createdAt: 1 }).lean(),
      Match.find({ $or: [{ userA: email }, { userB: email }] }).lean(),
      Report.find({ $or: [{ reporterEmail: email }, { reportedEmail: email }] }).lean()
    ]).catch(() => [[], [], []]);

    const payload = {
      exportedAt: new Date().toISOString(),
      format: "sharik-account-export/v1",
      profile: user,
    ...
    res.setHeader("Content-Disposition", 'attachment; filename="sharik-my-data.json"');
    res.json(payload);
  } catch (err) {
...
// ─── GDPR: حذف الحساب نهائياً (يتطلب كلمة المرور) ───────────────────
app.delete("/api/me", authMiddleware, async (req, res) => {
  try {
    const { password } = req.body || {};
    if (!password) return res.status(400).json({ error: "أدخل كلمة المرور للتأكيد" });
    ...
    // إزالة المحتوى المرتبط بالمستخدم
    await Promise.all([
      Message.deleteMany({ $or: [{ sender: email }, { receiver: email }] }),
      Match.deleteMany({ $or: [{ userA: email }, { userB: email }] }),
      Report.deleteMany({ $or: [{ reporterEmail: email }, { reportedEmail: email }] }),
      User.updateMany(
        { followers: email },
    ...
    // سجل تدقيق مجهول الهوية (بدون بيانات شخصية)
    await AuditLog.create({
    ...
    res.json({ success: true, message: "تم حذف الحساب وجميع بياناته نهائياً" });
  } catch (err) {
...
app.post("/api/upload/image", authMiddleware, async (req, res) => {
  try {
    const { image, folder = "sharik_uploads" } = req.body;
    if (!image) return res.status(400).json({ error: "بيانات الصورة مطلوبة" });

    const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
    const uploadPreset = process.env.CLOUDINARY_UPLOAD_PRESET || "sharik_preset";
    ...
    // Fallback: Optimized Base64 Data URL or CDN link
    if (image.length > 3.5 * 1024 * 1024) {
      return res.status(413).json({ error: "حجم الصورة كبير جداً (الحد الأقصى 3MB)" });
    }
    ...
    res.json({
      ...
      url: image,
      provider: "base64",
      message: "تم رفع وتجهيز الصورة بنجاح"
    });
  } catch (err) {
...
// GET /api/subscription/plans — جلب قائمة باقات الاشتراك المتاحة
    ...
    plans: [
      {
        ...
        badge: "أساسي",
        features: [
          "تصفح كل المهارات والمستخدمين",
          "حتى 3 طلبات مطابقة أسبوعياً",
          "محادثة كتابية غير محدودة",
          "الوصول للسبورة التفاعلية الأساسية"
        ],
        limits: { maxMatchesPerWeek: 3, aiMentorQueries: 5 }
      },
      {
        id: "pro",
        name: "المتقدمة (Pro)",
        ...
        popular: true,
        features: [
          "طلبات مطابقة غير محدودة ⚡",
          "شارة حساب موثق Pro ⭐",
          "استخدام محرر الكود والمكالمات 📹",
          "وصول غير محدود للمساعد الذكي AI",
          "أولوية الظهور في نتائج البحث 3X"
        ],
        limits: { maxMatchesPerWeek: 999, aiMentorQueries: 999 }
      },
      {
        id: "vip",
        name: "احترافية كبار الشخصيات (VIP)",
        nameEn: "VIP Elite",
        priceMonthly: 699,
        priceYearly: 6990,
        currency: "EGP",
        badge: "حصري VIP 👑",
        features: [
          "كل ميزات باقة Pro المتقدمة",
          "جلسات تدريب وتوجيه 1-on-1 مع خبراء",
          "مضاعفة نقاط الـ XP والـ Streak 2X",
          "شارة ذهبية ملكية VIP علي البروفايل",
          "مدير حساب وخدمة دعم فني مخصص 24/7"
        ],
        limits: { maxMatchesPerWeek: 999, aiMentorQueries: 999, vipCoaching: true }
      }
    ]
  });
});

// POST /api/subscription/checkout — تنفيذ وتأكيد اشتراك جديد / ترقية الباقة
app.post("/api/subscription/checkout", authMiddleware, async (req, res) => {
  try {
    const { plan, billingCycle = "monthly", paymentMethod = "card", cardNumber } = req.body;
    ...
    // Add notification
    const planNames = { pro: "المتقدمة (Pro) ⚡", vip: "كبار الشخصيات (VIP) 👑" };
    if (!user.notifications) user.notifications = [];
    user.notifications.push({
      title: "🎉 تم تفعيل اشتراكك بنجاح!",
      message: `مبروك! تم ترقية حسابك إلى باقة ${planNames[plan]} بنجاح لـ ${days} يوماً.`,
      type: "success",
      read: false,
      date: new Date()
    });
    ...
    // Add bonus XP
    const bonusXp = plan === "vip" ? 500 : 200;
    user.gamifyPoints = (user.gamifyPoints || 0) + bonusXp;
    ...
    await AuditLog.create({
      ...
      targetType: "User",
      targetEmail: user.email,
      metadata: { plan, billingCycle, paymentMethod, expiresAt }
    }).catch(() => {});
    ...
    res.json({
      success: true,
      message: `تم الترقية إلى باقة ${planNames[plan]} بنجاح!`,
      user: safeUser,
      subscription: user.subscription
    });
  } catch (err) {
    console.error("Checkout error:", err);
    res.status(500).json({ error: "فشلت عملية سداد الاشتراك" });
  }
});

// POST /api/subscription/cancel — إلغاء الاشتراك الحالي
app.post("/api/subscription/cancel", authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: "تم إلغاء الاشتراك والعودة للباقة المجانية",
      user: safeUser
    });
  } catch (err) {
...
app.post("/api/skills", authMiddleware, async (req, res) => {
  try {
    const learnSkills = normalizeSkillList(req.body.learnSkills);
    const teachSkills = normalizeSkillList(req.body.teachSkills);
    if (!learnSkills?.length || !teachSkills?.length) {
...
app.get("/api/skill-test/skills", authMiddleware, async (req, res) => {
  try {
    // Only skills the user actually selected AND that have a question bank —
    // the assessment must match the user's own choices and be startable.
    const user = await User.findById(req.userId).select("teachSkills learnSkills").lean();
    if (!user) return res.status(404).json({ error: "مستخدم غير موجود" });
    // المهارة قابلة للاختبار إن كان لها بنك (باسمها أو باسم بديل معروف)
    const testable = { has: (s) => isTestableSkill(s) };
    const skills = [
      ...new Set([...(user.teachSkills || []), ...(user.learnSkills || [])]),
    ].filter((s) => testable.has(s));
    res.json({ skills });
  } catch (err) {
...
app.post("/api/skill-test/submit", authMiddleware, async (req, res) => {
  try {
    ...
    if (sessionToken && Array.isArray(answers)) {
      ...
      const level =
        pct >= 90 ? "Expert" : pct >= 75 ? "Advanced" : pct >= 60 ? "Intermediate" : "Beginner";
      const update = {};
      ...
      safeUser.name = safeUser.username1 + " " + safeUser.username2;
      return res.json({ user: safeUser, passed, score, total, pct, level });
    }
...
app.post("/api/messages", authMiddleware, async (req, res) => {
  try {
    ...
    // الـ chatId يُشتق من الطرفين على الخادم — لا يُؤخذ من العميل
    const authoritativeChatId = Match.buildChatId(user.email, receiver);

    let message = await Message.findOne({ chatId: authoritativeChatId, messageId });
    if (!message) {
      message = new Message({
      ...
      io.to(authoritativeChatId).emit("newMessage", message);
    }
...
app.get("/api/user/:email", authMiddleware, async (req, res) => {
  try {
    ...
    const safeUser = attachGamification(user.toSafeObject());
    safeUser.name = safeUser.username1 + " " + safeUser.username2;
    // بيانات خاصة لا تُعرض لمستخدم آخر يتصفح الملف
    ...
    PRIVATE_FIELDS.forEach((k) => delete safeUser[k]);
    res.json({ user: safeUser });
  } catch (err) {
...
app.post("/api/reports", authMiddleware, async (req, res) => {
  try {
    const { targetEmail, reason, chatId, category, priority, evidence } = req.body;
    ...
    // Fetch reported user name if available
    const reportedUser = await User.findOne({ email: targetEmail.toLowerCase().trim() }).select("username1 username2").lean();
    const reportedName = reportedUser ? `${reportedUser.username1 || ""} ${reportedUser.username2 || ""}`.trim() : "";

    const allowedCategories = ["spam", "harassment", "fraud", "inappropriate", "fake_account", "other"];
    const allowedPriorities = ["low", "medium", "high", "critical"];

    const report = new Report({
      reporterEmail: reporter.email,
      reporterName: `${reporter.username1 || ""} ${reporter.username2 || ""}`.trim(),
      reportedEmail: targetEmail.toLowerCase().trim(),
      reportedName,
      reason: escapeHTML(String(reason).slice(0, 1000)),
      category: allowedCategories.includes(category) ? category : "other",
      priority: allowedPriorities.includes(priority) ? priority : "medium",
      evidence: evidence ? escapeHTML(String(evidence).slice(0, 500)) : "",
      chatId: chatId || ""
    });
...
app.get("/api/admin/users/:userId", requireAdmin("users:read"), async (req, res) => {
  ...
  res.json({
    user: {
      ...
      lastBrowser: lastLogin?.browser || "",
      lastOs: lastLogin?.os || "",
      loginHistory: (user.loginHistory || []).slice(-5).reverse(),
    },
    audit,
    reports,
  });
...
app.get("/api/admin/reports", async (req, res) => {
  try {
    ...
    const filter = {};
    if (["open", "pending", "reviewing", "reviewed", "resolved", "closed"].includes(status)) filter.status = status;
    if (["low", "medium", "high", "critical"].includes(priority)) filter.priority = priority;
    if (["spam", "harassment", "fraud", "inappropriate", "fake_account", "other"].includes(category)) filter.category = category;
    const [reports, total, openCount, highCount] = await Promise.all([
      Report.find(filter).sort({ priority: -1, createdAt: -1 }).skip((page - 1) * limit).limit(limit).lean(),
      Report.countDocuments(filter),
      Report.countDocuments({ status: { $in: ["open", "reviewing"] } }),
      Report.countDocuments({ priority: { $in: ["high", "critical"] }, status: { $nin: ["resolved", "closed"] } }),
    ]);
    res.json({
      reports,
      stats: { total, page, limit, totalPages: Math.max(1, Math.ceil(total / limit)), openCount, highPriorityCount: highCount },
    });
  } catch (err) {
    res.status(500).json({ error: "تعذر تحميل الشكاوى" });
  }
});
...
app.patch("/api/admin/reports/:id", requireAdmin("reports:write"), async (req, res) => {
  try {
    const patch = {};
    ["status", "adminNotes", "actionTaken", "priority"].forEach((key) => {
      if (Object.prototype.hasOwnProperty.call(req.body, key)) patch[key] = req.body[key];
    });
    ...
    // إرسال إشعار للمبلّغ عنه إذا تم الحل
    if (patch.status === "resolved" && report.reportedEmail) {
      await pushNotification(report.reportedEmail, {
        title: "تم مراجعة بلاغ بخصوص حسابك",
        message: patch.adminNotes || "تم مراجعة البلاغ المقدم بخصوص حسابك واتخاذ الإجراء المناسب.",
        type: "warning",
        read: false,
        date: new Date(),
      });
    ...
    res.status(500).json({ error: "تعذر تحديث الشكوى" });
  }
});

// حظر مستخدم مباشرة من صفحة الشكوى
app.post("/api/admin/reports/:id/ban-user", requireAdmin("users:write"), async (req, res) => {
  try {
    const report = await Report.findById(req.params.id);
    if (!report) return res.status(404).json({ error: "الشكوى غير موجودة" });
    const target = await User.findOne({ email: report.reportedEmail });
    if (!target) return res.status(404).json({ error: "المستخدم غير موجود" });
    const { status = "banned", reason = "", durationValue = null, durationUnit = "" } = req.body;
    if (!["banned", "suspended"].includes(status)) return res.status(400).json({ error: "حالة غير صحيحة" });
    let banUntil = null;
    if (durationValue && ["hours", "days", "weeks"].includes(durationUnit)) {
      const unitMs = durationUnit === "hours" ? 3600000 : durationUnit === "days" ? 86400000 : 604800000;
      banUntil = new Date(Date.now() + Number(durationValue) * unitMs);
    }
    target.status = status;
    target.banReason = String(reason || `إجراء بسبب بلاغ #${report._id}`).slice(0, 500);
    target.banUntil = banUntil;
    target.violationCount = Number(target.violationCount || 0) + 1;
    target.banHistory.push({ action: status, reason: target.banReason, until: banUntil, by: req.adminUser.email, at: new Date() });
    target.notifications.push({
      title: status === "suspended" ? "تم إيقاف حسابك مؤقتًا" : "تم تطبيق حظر على حسابك",
      message: target.banReason,
      type: "warning", read: false, date: new Date(),
    });
    await target.save();
    await Report.findByIdAndUpdate(report._id, { status: "resolved", actionTaken: `تم ${status === "banned" ? "حظر" : "إيقاف"} المستخدم`, handledBy: req.adminUser.email, handledAt: new Date() });
    await writeAuditLog(req, `user.${status}.from_report`, { type: "user", id: target._id, email: target.email }, { reportId: report._id, reason: target.banReason });
    res.json({ success: true, status: target.status });
  } catch (err) {
    res.status(500).json({ error: "فشل الإجراء" });
  }
});
...
app.post("/api/forgot-password", async (req, res) => {
  try {
    ...
    if (isEmailConfigured()) {
      ...
      // بدون SMTP وخارج الإنتاج: يُعاد الرابط للواجهة ليكمل المستخدم التدفق بنفسه
      console.log("[dev] Password reset link:", resetLink);
      return res.json({
        success: true,
        devResetLink: resetLink,
        message: "خدمة البريد غير مُعدّة — استخدم رابط إعادة التعيين مباشرة",
      });
    } else {
...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessage", async (data, ack) => {
    ...
    try {
      ...
      // غرفة العميل تُستخدم للتحقق من العضوية فقط — الحفظ والبث بالمعرّف المشتق من الطرفين
      const receiverEmail = String(data.receiver || "").toLowerCase().trim();
      const authoritativeChatId = Match.buildChatId(socket.user.email, receiverEmail);
      if (receiverEmail !== getPeerEmail(data.chatId, socket.user.email) || authoritativeChatId !== data.chatId) {
        ackErr(ack, traceId, "FORBIDDEN_CHAT", "forbidden");
        endEvent("sendMessage", startedAt, traceId);
        return;
      }
...
server.listen(port, "0.0.0.0", () => {
  ...
  // حالة خدمة البريد — ظاهرة عند التشغيل مباشرة لتجنب مفاجآت الاسترجاع/الإشعارات
  const emailMode = isEmailConfigured()
    ? "smtp ✔ (إيميلات حقيقية)"
    : (isProduction
      ? "غير مُعدّة ✘ — استرجاع كلمة المرور معطّل في الإنتاج! املأ SMTP_USER/SMTP_PASS في .env"
      : "وضع التطوير (بدون SMTP) — روابط الاسترجاع تُعاد في استجابة الـAPI");
