...
function calcMatchScore(currentUser, user) {
  ...
  const myLearn = currentUser.learnSkills || [];
  const myTeach = currentUser.teachSkills || [];
  const theirTeach = user.teachSkills || [];
  const theirLearn = user.learnSkills || [];
  const learnM = theirTeach.filter((s) => myLearn.includes(s)).length;
  const teachM = theirLearn.filter((s) => myTeach.includes(s)).length;
  const learnMV = theirTeach.filter(
    (s) => myLearn.includes(s) && uVerified.includes(s)
  ).length;
  const teachMV = theirLearn.filter(
    (s) => myTeach.includes(s) && myVerified.includes(s)
  ).length;
  const total = myLearn.length + myTeach.length || 1;
  let score = Math.min(
    100,
